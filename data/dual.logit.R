...
parameters.to.save <- c("b0","b1" , "sigma.y", "sigma.0", "sigma.1", "pi", "mu.0", "mu.1" , "pi.pp" , "mu.lr", "sigma.lrclass", "lrclassb", "mu.lrclassb" )

## missing data on lrclass. have to predict outside bugs
jags.data <- Hmisc::llist(yb, party.i, party.j, legis, n.rows, n.party, n.legis
                          , lrclassb
                          )
...
system.time(jags1 <- jags.model(file="model6pp.bug",data=jags.data, inits=inits, n.adapt=1000,n.chains=2))
...
system.time(jags2 <- jags.model(file="model7pp.bug",data=jags.data, inits=inits, n.adapt=1000,n.chains=2))

system.time(jags2.s <- coda.samples(jags2,variable.names=parameters.to.save, n.iter=10000 , thin=10))


jags1.bugs <- coda2bugs(jags1.s)
jparty <- with(jags1.bugs$mean, data.frame(party=1:n.party, pi=pi, pi.pp=pi.pp ))
jself <- with(jags1.bugs$mean, data.frame(legis=1:n.legis, lrclassb, mu.lr, b0, b1, mu.lrclassb ))


jags2.bugs <- coda2bugs(jags2.s)
jparty <- with(jags2.bugs$mean, data.frame(party=1:n.party, pi=pi, pi.pp=pi.pp ))
jself <- with(jags2.bugs$mean, data.frame(legis=1:n.legis, lrclassb, mu.lr, b0, b1, mu.lrclassb ))
