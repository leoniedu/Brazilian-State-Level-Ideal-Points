...
parameters.to.save2 <- c("b0","b1" , "sigma.y", "sigma.0", "sigma.1", "pi", "mu.0", "mu.1" , "pi.pp", "mu.lrclass", "sigma.lrclass" )
...
lrclass.pp <- (coda2bugs(jags1.s, parms.tokeep="lrclass")$sims.list$lrclass-shift)*scale
...
party <- coda2bugs(jags1.s, parms.tokeep="party")
b0 <- coda2bugs(jags1.s, parms.tokeep="b0")
b1 <- coda2bugs(jags1.s, parms.tokeep="b1")
